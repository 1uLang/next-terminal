...
var anonymousUrls = []string{"/login", "/static", "/favicon.ico", "/logo.svg", "/branding", "/asciinema", "/authorize-token"}
...
func Auth(next echo.HandlerFunc) echo.HandlerFunc {
	...
	return func(c echo.Context) error {
		...
		if token == "" {
			return api.Fail(c, 401, "您的登录信息已失效，请重新登录后再试。1")
		}

		v, found := cache.TokenManager.Get(token)
		if !found {
			return api.Fail(c, 401, "您的登录信息已失效，请重新登录后再试。2")
		}
...
func Admin(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if !found {
			return api.Fail(c, 401, "您的登录信息已失效，请重新登录后再试。4")
		}
